    ...
    if (editOptions) {
        ...
        const SuggestionsPopover = (
            <Popover id="suggestions-popover" className={suggestions.length === 0 ? 'hidden' : ''}>
                <Popover.Content className="tag-sugesstions">
                    {suggestions.map((tag, i) => (
                        <Badge className="pointer" key={i} onClick={() => onSuggestionClicked(tag, i)} variant="light">
                            {tag}
                        </Badge>
                    ))}
                </Popover.Content>
            ...
            <React.Fragment>
                <div className="editable-tag-list">
                    <ul className="editable-tag-list-tags">
                        {tags.map((tag, i) => (
                            <li key={i}>
                                <Badge className="tag-pill" variant="dark">
                                    {tag}
                                    <FontAwesomeIcon
