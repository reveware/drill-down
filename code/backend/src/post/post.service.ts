...
export class PostService {
    ...
    private static getMongooseFilterQuery(user: Populated<User>, params: GetPostsFiltersDTO = {}): mongoose.FilterQuery<PostDocument> {
        ...
        const query = _.reduce(
            ...
            {} as mongoose.FilterQuery<PostDocument>
        );
    ...
    public async createPhotoPost(
        ...
        const populatedPost = _.first(await this.getPosts(user, { _id: newPost._id }));
    ...
    public async getPostsCountByTag(user: Populated<User>): Promise<CountByTag[]> {
        return new Promise(async (resolve, reject) => {
            return await this.postModel.aggregate(
                ...
                (err: Error, result: {_id: string, count: number}[]) => {
                    if (err) {
                        reject(err);
                    }
    ...
    public async createComment(postId: string, comment: Unpopulated<Comment>): Promise<Populated<Comment>> {
