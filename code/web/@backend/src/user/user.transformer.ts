import { CountPerTag, UserDetail, UserOverview, UserRole } from '@drill-down/interfaces';
...
export class UserTransformer {
    ...
    public static toUserDetail(user: PrismaDetailResult, postCountByTag: CountPerTag): UserDetail {
        const overview = UserTransformer.toUserOverview(user);

        const tags = _.keys(postCountByTag)
        const sortedCount = _.sortBy(tags, (tag)=> postCountByTag[tag])
